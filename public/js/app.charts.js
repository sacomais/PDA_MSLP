  ...
  function buildHourlyChart(llegadas, salidas) {
    ...
    S.charts.hourly = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          tooltip: tooltipCommon, // Modificaremos esto en el Paso 2
          legend: { position: 'top', labels: { usePointStyle: true, boxWidth: 18, boxHeight: 10 }, onClick: null },

          // --- NUEVO: ETIQUETAS DE TOTAL SOBRE LAS BARRAS ---
          datalabels: {
            // Solo dibujamos la etiqueta en el bloque de 'Salidas' (index 1) para que quede en la cima de la barra apilada
            display: function(context) {
                return context.datasetIndex === 1;
            },
            formatter: function(value, context) {
                // Sumamos Llegadas (index 0) + Salidas (index 1) de esa hora específica
                const valLlegadas = context.chart.data.datasets[0].data[context.dataIndex] || 0;
                const valSalidas = context.chart.data.datasets[1].data[context.dataIndex] || 0;
                const total = valLlegadas + valSalidas;
                
                // Si el total es mayor a 0, muestra el número; si es 0, lo deja en blanco
            ...
            offset: 2 // Pequeña separación entre la barra y el número
          }
          // ---------------------------------------------------
        },
        onClick: (evt, elements) => {
          if (elements && elements.length) {
            ...
            // Se eliminó la dependencia de filtroHora
            window.APP_MAIN.procesarYRender();
          }
        }
      },
      plugins: [capacidadLabelPluginHora, ChartDataLabels] // Este es tu plugin original, lo mantenemos intacto
    });
}
